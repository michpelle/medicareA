...
for (y in 2008:2015) {
  ...
  for (m in monthlist) {
    ...
    ma.path=paste0(path.data.ma,"/monthly-ma-state-and-county-penetration/State_County_Penetration_MA_",y,"_",m,".csv")
...
write_tsv(ma.penetration.data,path=paste(here("/MA_Penetration.txt",sep="")),append=FALSE,col_names=TRUE)
write_rds(ma.penetration.data,paste(here("/ma_penetration.rds",sep="")))